...
if ( !JsNamespace.exist("Eco.XComp.Factory") )
{
	...
	*/
	...
	JsNamespace.declare("Eco.XComp.Factory", {
		...
		/**
		 * form(div, tabpage포함)에 종속된 comp 중에서 사용되고 있는 것에 해당하고,<br>
		 ...
		 * @return {XComp} name component name.
		 * @example
		...
		/**
		 * form(div, tabpage포함)에 설정되어진 cache array를 얻는  함수<br>
		 ...
		 * @return {array}  사용하지 않고 cache된 component array.
		 * @private
		 * @memberOf Eco.XComp.Factory
		 */
		...
		/**
		 * form(div, tabpage포함)에 설정되어진 사용하고 있는 comp들의 collection를 얻는  함수.
		...
		/**
		 * form(div, tabpage포함)에 종속된 component를 얻는 함수<br>
		 * 세번째 인자 nm 으로 comp._id을 설정한다. 필요시에는 cache된 comp를 재 사용한다.
		 * @param {form|div|tabpage|object} p form에 해당하는 XComp 또는 p.target, p.poolName가지는 object.
		 * @param {string} type XComp type.
		 * @param {string} nm 설정하고자 하는 component._id.
		 * @return {XComp} 동적으로 생성되거나, cache에서 재 사용된 component.
		 * @example
		...
		/**
		 * type에 해당하는 생성자를 얻는 함수.
		 * @param {string} type XComp type.
		 * @return {xcomp constructor} XComp Constructor
		 * @private
		 * @memberOf Eco.XComp.Factory
		 */
		...
		/**
		 * XComp 재활용을 위해 visible 속성을 false로 설정한다.
		 * @param {XComp} c XComp.
		 * @param {boolean=} clear 정의된 속성값 clear 여부.
		 * @private
		 * @memberOf Eco.XComp.Factory
		 */
		 ...
		 * @param {Eco.CssItem} CssItem {@link Eco.CssItem} 스타일 속성정보
		 * @return {array} [너비, 높이]
		 * @memberOf Eco.XComp.Factory
		 */		 
		...
		/**
		 * XComp의 rect 정보를 설정한다.<br>
		 * Eco.XComp.Factory.doLayout 함수를 호출하기 전까지 화면상에 변경이 발생하지 않는다.
		...
		/**
		 * XComp의 속성 값을 설정한다.<br>
		 * Eco.XCompFactory.doLayout 함수를 호출하기 전까지 화면상에 변경이 발생하지 않는다.
		 * 설정된 정보를 _props에 가지고 있다가 doLayout함수 호출하는 시점에 반영하고 _props정보는 clear된다.
		 * @param {XComp} c XComp.
		 * @param {...*} property name, property value 순으로 정의되는 인자.
		 * @memberOf Eco.XComp.Factory
		 */
		...
		/**
		 * form 객체에 속한 component 중에 변경된 속성 및 rect를 실제로 화면상에 반영한다.
		 * @param {form|div|tabpage} p form|div|tabpage type of XComp.
		 * @param {boolean=} isDownward 하위 form까지 처리할 것인지 여부(default: false).
		 * @memberOf Eco.XComp.Factory
		 */
		"doLayout": function(p, isDownward)
		{
			if(p instanceof nexacro.Div){
				p = p.form;
			}
		...
		/**
		 * form 객체에 속한 component 중에 속성 값이 변경되어진 목록인 [_commitPropItems] array를<br>
		 * 가지고 속성을 실제로 반영하는 처리를 한다.
		 * @param {form|div|tabpage} p form|div|tabpage type of XComp.
		 * @param {boolean=} isDownward 하위 form까지 처리할 것인지 여부(default: false).
		 * @private
		 * @memberOf Eco.XComp.Factory
		...
		/**
		 * form 객체에 속한 component 중에 rect 값이 변경되어진 목록인 [_renderItems] array를<br>
		 * 가지고 변경된 rect을 실제로 반영하는 처리를 한다.
		 * @param {form|div|tabpage} p form|div|tabpage type of XComp.
		 * @param {boolean=} isDownward 하위 form까지 처리할 것인지 여부(default: false).
		 * @private
		 * @memberOf Eco.XComp.Factory
		...
		/**
		 * 주어진 clist array 목록 순으로 zorder를 반영한다.
		...
		/**
		 * XComp의 속성 및 rect 변경되어 차후 반영할 것을 예약 처리한다.<br>
		 * XComp.parent의 _renderItems, _commitPropItems array에 각각 등록한다.
		 * @param {XComp} c XComp.
		 * @memberOf Eco.XComp.Factory
		 */
		...
		/**
		 * XComp의 속성 및 rect 변경되었다고 기록한 후 즉시 doLayout를 호출하여 반영처리한다.
		 * @param {XComp} c XComp.
		 * @memberOf Eco.XComp.Factory
		 */
		"validate": function(c)
		...
		/**
		 * XComp의 rect 변경되어 차후 반영할 것을 예약 처리한다.<br>
		 * XComp.parent의 _renderItems array에 등록한다.
		 * @param {XComp} c XComp.
		 * @memberOf Eco.XComp.Factory
		 */
		...
		/**
		 * XComp의 rect 변경되었다고 기록한 후 즉시 doLayout를 호출하여 반영처리한다.
		 * @param {XComp} c XComp.
		 * @memberOf Eco.XComp.Factory
		 */
		...
		/**
		 * XComp의 상위 component에 rect(position, size)가 변경된 child가 존재하다고 예약 처리한다.<br>
		 * XComp.parent의 _changesRectChilds array에 등록한다.<br>
		...
		/**
		 * XComp의 상위 component에 rect(position, size)가 변경되었다고 예약된 child를 삭제한다.<br>
		 * XComp.parent의 _changesRectChilds array에 삭제한다.<br>
		...
		/**
		 * XComp의 속성이 변경되어 차후 반영할 것을 예약 처리한다.<br>
		 * XComp.parent의 _commitPropItems array에 등록한다.<br>
		 * XComp의 변경할 XComp._props 값과 변경된 XComp._curProps 값이 동일하면 <br>
		 * 속성 변경할 목록인_commitPropItems[]에서 제거한다.
		 * @param {XComp} c XComp.
		 * @memberOf Eco.XComp.Factory
		 */
		...
		/**
		 * XComp의 상위 component에 property가 변경된 child가 존재하다고 예약 처리한다.<br>
		 * XComp.parent의 _changesPropChilds[]에 등록한다.<br>
		 * recursion으로 parent에 예약 처리하면서 ChildFrame에서 멈춘다. 
		 * @param {XComp} c XComp.
		 * @memberOf Eco.XComp.Factory
		 */
		...
		/**
		 * XComp의 상위 component에 property가 변경되었다고 예약된 child를 삭제한다.<br>
		 * XComp.parent의 _changesPropChilds[]에 삭제한다.<br>
		 * recursion으로 parent에 삭제 처리하면서 ChildFrame에서 멈춘다. 
		 * @param {XComp} c XComp.
		 * @memberOf Eco.XComp.Factory
		 */
		...
		/**
		 * XComp의 속성이 변경되었다고 기록한 후 즉시 doLayout를 호출하여 반영처리한다.
		 * @param {XComp} c XComp.
		 * @memberOf Eco.XComp.Factory
		 */
		...
		/**
		 * rect정보가 변경된 목록인 _renderItems[]에 주어진 XComp를 추가한다.
		 * @param {XComp} c XComp.
		 * @private
		 * @memberOf Eco.XComp.Factory
		 */
		...
		/**
		 * 속성 정보가 변경된 목록인 _commitPropItems array에 XComp를 추가한다.
		 * @param {XComp} c XComp.
		 * @private
		 * @memberOf Eco.XComp.Factory
		 */
		...
		/**
		 * rect정보가 변경된 목록인 _renderItems[]에 주어진 XComp를 제거한다.
		 * @param {XComp} c XComp.
		 * @private
		 * @memberOf Eco.XComp.Factory
		 */
		...
		/**
		 * 속성 정보가 변경된 목록인 _commitPropItems[]에 주어진 XComp를 제거한다.
		 * @param {XComp} c XComp.
		 * @private
		 * @memberOf Eco.XComp.Factory
		 */
		...
		/**
		 * XComp._props는 변경할 속성들의 정보 값인데 이것을 실제로 XComp에 반영하는 처리.
		 * @param {XComp} c XComp.
		 * @private
		 * @memberOf Eco.XComp.Factory
		 */
			...
			if ( c._needsCommitProps )
			{
				...
				if ( props )
				{
					for ( var pNm in props )
					{
						...
						{
							curVal = props[pNm];
							if(pNm == 'style'){ // 파싱임시코드 추후 수정 필요
								var vType = typeof curVal;
								switch(vType){
									case 'string':
										var styleArr = curVal.split(';');
										for(var i = 0, len = styleArr.length; i < len; i++){
											var propStr = styleArr[i];
											if(propStr.length > 0){												
												var tmpidx = propStr.indexOf(':');
												var pKey = propStr.substr(0, tmpidx);
												pKey = pKey.replace(/ /g, '');									
												var pValue = propStr.substr(tmpidx+1, propStr.length);
												if(c['set_' + pKey]){
													c['set_' + pKey](pValue);
												}
											}
										...
										break;
									case 'object':
										Eco.XComp.Style.setStyle(xComp, curVal);
										break;										
									default:
										break;
		...
		/**
		 * XComp._rect는 변경할 rect 정보 값인데 이것을 실제로 XComp에 반영하는 처리.
		 * @param {XComp} c XComp.
		 * @private
		 * @memberOf Eco.XComp.Factory
		 */
		...
		/**
		 * form,div,tabpage 객체에 속하고, 주어진 명칭 값을 담고 array에 해당하는 component를 모두 cache 처리한다.<br>
		 * doLayout 함수 호출 전까지는 화면상에 보여진다.<br>
		 * 하지만 화면상에 반영되는 것과 별개로 cache된 기록은 내부적으로 처리된다.<br>
		 * 첫번째 인자가 object로 주어질때는 poolName를 임의로 지정하여 사용하는 경우이다.
		 * @param {form|div|tabpage|object} p form XComp 또는 object{target: form객체, poolName: "rowobjs"}.
		 * @param {array} names cache처리할 대상를 component._id의 array.
		...
		/**
		 * form 객체에 속한 사용중인 component를 모두 cache 처리한다.<br>
		 * doLayout 함수 호출 전까지는 화면상에 보여진다.<br>
		 * 하지만 화면상에 반영되는 것과 별개로 cache된 기록은 내부적으로 처리된다.<br>
		 ...
		 * @param {boolean=} propsClear cache처리할 때 기 설정된 속성값들을 clear 할 것인지 여부.
		...
		/**
		 * XComp에 대하여 cache처리한다.<br>
		 * doLayout 함수 호출 전까지는 화면상에 보여진다.<br>
		 * 하지만 화면상에 반영되는 것과 별개로 cache된 기록은 내부적으로 처리된다.<br>
		 * @param {XComp} c cache처리할 XComp.
		 * @param {string=} poolName 임의 poolName에 대하여 처리하고자 할 때 사용하는 pool 명칭.
		 * @param {boolean=} propsClear cache처리할 때, 기 설정된 속성값들을 clear 할 것인지 여부.
		 * @memberOf Eco.XComp.Factory
		 */
		...
		/**
		 * XComp의 pseudo에 설정된 값을 기준으로 nonClient영역에 해당하는 left, top, right, bottom 값을 array로 반환한다.<br>
		...
		/**
		 * 문자열을 parsing하여 Style object들을 collection으로 반환한다.<br>
		 * collection는 pseudo별로 background, border 등 name별로 객체들을 담아둔다.
		 * @param {string} str style 설정할 때 사용하는 string형태.
		 * @return {object} 문자열을 parsing하여 생성된 Style object들의 collection.
		 * @memberOf Eco.XComp.Factory
		 */
