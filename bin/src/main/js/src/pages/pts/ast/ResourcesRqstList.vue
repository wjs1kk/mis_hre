    ...
    grdRqstList = new dhx.Grid(null, {
        ...
        columns: [
            ...
            },
            {
                id: "astUseYmd",
                gravity: 90,
                header: [{ text: "사용기간", align: "center" }],
                editable: false
            },
            {
                id: "astRtnDueYmd",
                gravity: 90,
                header: [{ text: "반납예정일자", align: "center" }],
                editable: false
            },
            {
                id: "rgstDt",
                gravity: 90,
                header: [{ text: "신청일자", align: "center" }],
                editable: false
            }
        ]
    });

    grdRqstListLayout = new dhx.Layout(gridElement.value, {
        rows: [
            { id: "grid" }
        ]
    });

    grdRqstListLayout.getCell("grid").attach(grdRqstList);

    grdRqstList.events.on("cellClick", editOnSingleClick);

    grdRqstList.events.on("cellDblClick", (row, col) => {
        if (col.id === "astNo") {
            globalUi.openTab({
                tabId: "resourcesRqstManage",
                tabTitle: "자원 신청",
                url: "pages/pts/ast/ResourcesRqstManage.vue",
                args: {
                    astNo: row.astNo
                }
            })
...
// 컴포넌트 언로드
onUnmounted(() => {
    grdRqstList?.destructor();
    grdRqstListLayout?.destructor();
...
async function search() {
    const result = await postData("ast/resources/getAstRqst.do", {
        dsSearch: searchForm.getValue()
    });


    grdRqstList.data.parse(result.dsList);
}
    ...
    <div class="jn-page">
        ...
        <div class="jn-container" style="height: calc(100% - 143px);">
            <div class="jn-title-container">
                ...
                <div class="jn-btn-group">
                    <button class="jn-btn-form icon-add" @click="addAst">자원 신청</button>
                </div>
            </div>
