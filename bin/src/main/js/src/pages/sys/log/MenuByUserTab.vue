...
const dsMenuByUser = new DataCollectionEx();
...
async function search(searchData) {
    const dsSearch = datasetConverter(searchData)
        ...
        .convert();
    const result = await postData("sm/progStat/getMenuByUserStat.do", { dsSearch: dsSearch });
    dsMenuByUser.parse(result.dsMenuByUser);
}
