...
const dsUserByMenu = new DataCollectionEx();
...
async function search(searchData) {
    const dsSearch = datasetConverter(searchData)
        ...
        .convert();
    const result = await postData("sm/progStat/getUserByMenuStat.do", { dsSearch: dsSearch });
    dsUserByMenu.parse(result.dsUserByMenu);
}
