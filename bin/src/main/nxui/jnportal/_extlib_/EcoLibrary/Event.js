...
if ( !JsNamespace.exist("Eco.XComp.Event") )
{
	...
	JsNamespace.declare("Eco.XComp.Event", {
		...
		**/		
		 ...
		 * @param {object|function} draggingFunc dragging 처리 루틴에 해당하는 함수들.<br>
		 * <pre>
         *    argument가 function이면 dragging 되는 시점에 호출되는 함수로 설정된다.
		 ...
		 *    
		 *    offsetX, offsetY param 값은 dragging이 발생되는 동안 
		 *    현재상태에서 이동할 x,y 좌표의 차이이다.
		 *    만약 5번째 인자값(isOffsetFromStart)이 true이면
		 *    최초 drag 시점의 x, y 기준으로 dragging으로 이동할 x, y 값의 차이가 주어진다.
		 * </pre>
		 * @param {*=} scope scope는 호출되는 함수 내부의 this 로 지정된다(default: form).
		...
		**/
		 ...
		 *
		 * @param {XComp} XComp draggable 기능을 해제하는 xcomp.
		 * @memberOf Eco.XComp.Event
		**/
		...
		**/
		 ...
		 * @param {Form} form event가 발생한 top form.
		 * @param {EventInfo} e EventInfo 객체.
		 * @private
		 * @memberOf Eco.XComp.Event
		**/
		...
		*/
		"_on_default_sys_lbuttonup": function(elem, strButton, altKey, ctrlKey, shiftKey, windowX, windowY, screenx, screeny)
		{
			...
			nexacro._Window.prototype._on_default_sys_lbuttonup.call(this, elem, strButton, altKey, ctrlKey, shiftKey, windowX, windowY, screenx, screeny);
			this._on_sys_lbuttonup = nexacro._Window.prototype._on_default_sys_lbuttonup;
			this._on_sys_mousemove = nexacro._Window.prototype._on_default_sys_mousemove;
		...
		*/
		"_on_default_sys_mousemove": function(elem, strButton, altKey, ctrlKey, shiftKey, windowX, windowY, screenx, screeny)
		{
			...
			nexacro._Window.prototype._on_default_sys_mousemove.call(this, elem, strButton, altKey, ctrlKey, shiftKey, windowX, windowY, screenx, screeny);
		},
		
		/**
		 * 주어진 XComp에 repeat 기능를 설정한다.
		 * 
		 * @param {XComp} XComp repeatable하고자 하는 XComponent.
		...
		**/
		...
		/**
		 * repeating 처리를 위한 onlbuttondown event handler.
		 * @param {XComp} obj 발생한 event의 XComp.
		 * @param {EventInfo} e EventInfo 객체.
		 * @private
		 * @memberOf Eco.XComp.Event
		**/
		...
		/**
		 * repeating 처리를 위한 onlbuttonup event handler.
		 * @param {XComp} obj 발생한 event의 XComp.
		 * @param {EventInfo} e EventInfo 객체.
		 * @private
		 * @memberOf Eco.XComp.Event
		**/
		...
		/**
		 * repeating 처리를 위한 onmousemove event handler.
		 * @param {XComp} obj 발생한 event의 XComp.
		 * @param {EventInfo} e EventInfo 객체.
		 * @private
		 * @memberOf Eco.XComp.Event
			...
			if ( !rAF )
			{
				...
				if ( isRuntime )
				{
					...
					rAF = function(form, callback, lastTimeRef) {
						...
						var id = nexacro._OnceCallbackTimer.callonce(form, function(){
