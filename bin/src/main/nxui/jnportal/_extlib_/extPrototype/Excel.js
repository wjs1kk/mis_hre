...
{	
	...
	//var svcUrl = "svcurl::XExportImport.do";
	var sSrvUrl = nexacro.getEnvironment().services["svcurl"].url;  // service URL
	var svcUrl = sSrvUrl + "XImport";
...
{		
	...
	//화면의 callback 함수 호출
	if (!this.gfnIsNull(sCallback)) this.lookupFunc(sCallback).call(objForm,sImportId);
};
...
pForm.gfnExcelImport = function(sDataset,sSheet,sBody,sCallback, sImportId, objForm)
{
	...
	// set import url ex) http://localhost:8080/kritpms/XImport
	var sSrvUrl = nexacro.getEnvironment().services["svcurl"].url;  // service URL
	var svcUrl = sSrvUrl + "XImport";
	...
	if(this.isValidObject(sOutDsName)){
		var rDs = this.removeChild(sOutDsName);
		if(rDs != null) rDs.destroy();
	}
...
/**
 * @class excel import on success <br>
 * @param {Object} obj ExcelImportObject	 
 * @param {Event} e	ExcelImportEventInfo	 
 * @return N/A
 * @example
 * @update 2023.08.05 parkmg 기존 copyData 방식을 copyRow 로 변경
 *           엑셀의 칼럼수보다 objOrgDs 의 칼럼수가 작을 경우 
 *	         objOrgDs 칼럼이 사라지는 문제점 보완
 */
...
{		
	...
	for(var i = 0; i < objOutDs.colcount; i++){
	...
	for(var i = 0; i < objOutDs.rowcount; i++){
		var nRowId = objOrgDs.addRow();
		objOrgDs.copyRow(nRowId, objOutDs, i);
	}
	
	var oOutds = this.removeChild(objOutDs.name);
	oOutds.destroy();
