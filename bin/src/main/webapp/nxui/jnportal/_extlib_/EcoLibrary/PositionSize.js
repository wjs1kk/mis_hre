...
if ( !JsNamespace.exist("Eco.XComp.PositionSize") )
{
	...
	JsNamespace.declare("Eco.XComp.PositionSize", {
		 ...
		 * @example
		 * trace(Eco.XComp.PositionSize.getTopLevelFormBox(Div00.form.Button00)); //output: [829,135,63,43]
		 * trace(Eco.XComp.PositionSize.getTopLevelFormBox(Div01.form.Div02.form.Button01)); //output: [841,346,63,43]
		 * trace(Eco.XComp.PositionSize.getTopLevelFormBox(Tab00.tabpage1.form.Button02)); //output: [813,521,63,43]	
		 *
		...
		getScrollBarSize: function(XComp, type)
		{
			...
			return type == "horz" ? XComp._getHScrollBarSize() : XComp._getVScrollBarSize();
		},		
		...
		{
			//(text, font, width, wordwrap, wordspacing, letterspacing, lineheight)
			var font;
			var size = [0, 0];
			
			if ( XComp instanceof nexacro._FontObject )
			{
				font = XComp;	
			...
			else
			{
				...
				if ( Eco.isEmpty(font) )
				{					
					font = XComp._getCurrentStyleInheritValue('font');
				}				
			...
			if ( nexacro._Browser != "Runtime" )
			{
				...
				if ( nexacro._Browser == "IE" && nexacro.BrowserVersion < 9 )
				{
		...
		getContentSize: function(XComp, width, multiLine)
		{
			...
			{		
				...
				var comp = XComp instanceof nexacro.Div ? XComp.form : XComp;

				// scroll을 가지는 컴포넌트는 현재 자신의 size 를 리턴
				if ( !Eco.isEmpty(comp.hscrollbar) && !Eco.isEmpty(comp.vscrollbar) )
				{
				...
				else
				{
					...
					if ( type == "Calendar" )
					{
						...
						{
							//dropbutton
							var dropbutton = XComp.dropbutton;
							if(dropbutton) {
								w += dropbutton.width;
								clientHeight = dropbutton.height;
							}
							
							if(clientHeight < sz[1]) {
								clientHeight = sz[1];
							}
					...
					else if ( type == "Combo" )
					{
						...
						sz = this._getStyleSpaceSize(XComp.comboedit);
						w += sz[0];
					...
					else if ( type == "Radio" )
					{
						...
						var itempadding = XComp._itempadding;
					...
					if ( type == "CheckBox" )
					{
						textpadding = XComp._cell_elem.textPadding;
						var checkImg = XComp._cell_elem._box_node.children[0];
						if ( checkImg ) {	
							clientHeight = checkImg.clientHeight;
							clientWidth  = checkImg.clientWidth;
						}
					}
		...
		getPositionByForm: function(XComp, width, height, direction, offset, scopeXComp)
		{
			...
			if(scopeXComp) {
				...
				relativeXComp = nexacro.getApplication().mainframe;
			}
