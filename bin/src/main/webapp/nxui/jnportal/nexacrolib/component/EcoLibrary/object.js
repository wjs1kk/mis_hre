...
if ( !JsNamespace.exist("Eco.object") )
{
	...
	JsNamespace.declare("Eco.object",{
		...
		*/
		...
		/**
		* object에 주어진 argument들로 속성값들을 합치는 처리.<br>
		...
		* @param {object...} target object와 합치는 object들.
		* @param {function=} func callback 함수.
		* @param {object=} scope callback 함수에 대한 수행 scope.
		...
		*/
		...
		/**
		* argument로 주어진 object의 속성 명칭과 속성 값을 하나의 Object로 구성하여 Array로 넘겨준다. 
		* @param {object} object 대상 객체.
		* @return {array} {name:value} 배열.
		* @example
		...
		*/
		...
		/**
		* object에 속성들(name, value)을 dataset에 구성한다.<br>
		...
		* @param {object} object Dataset 구성에 참조되는 객체.
		* @param {dataset} ds 객체 속성,값을 참조해서 구성되는 Dataset.
		* @param {string=} dir direction(horz:default,vert).
		* @example
		* var info = { "basic": "A",
		* 	"string": "가",
		* 	"int": 10,
		...
		*/
		...
		/**
		* object에 속성들(name, value)을 dataset로 부터 구성한다.<br>
		* dir 값이 vert 이면 dataset에 name Column, value Column 필드명에서<br>
		* record별로 정보을 얻은 후 object로 구성한다.<br>
		* dir 값이 horz 이면 dataset의 Column들의 id를 속성 명칭으로 사용하고<br>
		* 각 Column의 0번째 row 값을 object value로 구성한다.
