...
<%@page import="com.jnv.jncore.util.ApplicationContextHolder" %>
<%@ page contentType="text/html;charset=utf-8"%><%
    //response.setHeader("Access-Control-Allow-Methods", "POST, GET, OPTIONS");
    //response.setHeader("Access-Control-Allow-Headers", "content-type, RAONK-Encoded");
    //response.setHeader("Access-Control-Allow-Origin", "*");

    Logger logger = LoggerFactory.getLogger("raonkhandler.jsp");
    Properties properties = ApplicationContextHolder.getBean("systemProp", Properties.class);
    String uploadRoot = properties.getProperty("rootFilePath");
    String tempDir = properties.getProperty("tempFilePath");
    ...
    if (System.getProperty("os.name").toLowerCase().contains("windows")) {
        uploadDir = Paths.get("C:\\", uploadDir).toString();
        tempDir =  Paths.get("C:\\", tempDir).toString();
    }
    ...
    String[] pgmId = upload.getParameterValue("pgmId");
    if (pgmId != null && pgmId.length > 0) {
        uploadDir = Paths.get(uploadDir, pgmId).toString();
    }
    ...
    event.addAfterUploadEventListener(new AfterUploadEventListener() {
        public void afterUploadEvent(EventVo eventVo) {
            ...
            try {
                ...
                //logger.error("Error occured on AfterUpload event", ex);
            }
        }
    });
    */
    ...
    */
    ...
    upload.settingVo.setPhysicalPath(uploadDir);
    upload.settingVo.setTempPath(tempDir);
    //-------------------- [설정방법1] 물리적 경로 설정 끝 --------------------//
